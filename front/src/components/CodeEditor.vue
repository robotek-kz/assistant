...
export default {
  ...
  methods: {
        ...
        if (this.countdown === 2) {
          if (!this.editorsReady || !this.originalCode) {
            ...
            if ((Date.now() - initTime) / 1000 < 5) {
              this.popUpText = 'Ожидание загрузки...';
            } else {
              console.warn('Long loading time');
              this.popUpText = 'Что-то случилось.';
            }
          }
      ...
      Promise.all([this.getCode(), loadTheme(this.options.selectedTheme), loadMode(this.originalCm, this.language.mode)])
        ...
        .catch((err) => {
          ...
          if (err.message === 'No internet') {
            this.popUpText = 'Отсутсвтует соединения с сервером.';
          } else {
            this.popUpText = 'Пожалуйста попробуйте позже';
          }
        });
    },
    completed(currentStats = true) {
      ...
      this.popUp(true, 'MISSION COMPLETED');
      ...
      console.log('editorCode', this.editorCode);
      ...
      // };
      // axios.post('http://localhost:5000/api/sketches', this.editorCode, {
      //   headers,
      // })
      //   .then((res) => {
      //     console.log(res);
      ...
      //     console.log(res);
      //   });
      // axios.post('http://localhost:1024/send', this.stats)
      //   .then((res) => {
      //     console.log(res);
      //   })
      //   .catch((res) => {
